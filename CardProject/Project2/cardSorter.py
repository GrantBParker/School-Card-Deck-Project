...
class CardSorter():
    ...
    def construct_piles_subseqs(self):
        ...
        for c in range(self.CARDS_IN_DECK):
            ...
            for i in range(len(self.__piles)):
                topCard = self.__piles[i].get_top_card()
                if topCard.compare(card) > 0:
                    cardAdded = True
                    if i > 0:
                        previousCard = self.__piles[i - 1].get_top_card()
                        self.add_card_to_subseq(card, previousCard)
                    self.__piles[i].add_card(card)
                    break
            if cardAdded is False:
                pile = Pile()
                if len(self.__piles) > 0:
                    previousCard = self.__piles[len(self.__piles) - 1].get_top_card()
                    self.add_card_to_subseq(card, previousCard)
                pile.add_card(card)
    ...
    def make_sorted_deck(self):
        ...
        for i in range(self.CARDS_IN_DECK):
            ...
            for k in range(len(self.__piles)):
                if len(self.__piles[k]) > 0:
                    card = self.__piles[k].get_top_card()
                    if card.compare(low_card) <= 0:
                        low_card = card
                        low_pile = k

            card = self.__piles[low_pile].remove_card()
